						  ...
						  float screenDepth, float screenNear)
{
	...
	result = CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, __uuidof(IDXGIFactory4), (void**)&factory);
	if (FAILED(result))
	{
		return false;
	}
	...
	m_vp.Height = static_cast<float>(screenHeight);
	m_vp.Width = static_cast<float>(screenWidth);
	m_vp.TopLeftX = 0.0f;
	m_vp.TopLeftY = 0.0f;
	m_vp.MaxDepth = D3D12_MAX_DEPTH;
	m_vp.MinDepth = D3D12_MIN_DEPTH;
	// Set up the rect
	m_sr.left = 0;
	m_sr.top = 0;
	m_sr.bottom = static_cast<LONG>(screenHeight);
	m_sr.right = static_cast<LONG>(screenWidth);
...
void D3DClass::SetRootSignature(ID3D12RootSignature* signature)
{
	m_prtSignature = signature;
}

void D3DClass::SetRenderState(ID3D12PipelineState* state)
{
	m_pipelineState = state;
}

bool D3DClass::FinishUp()
{
	auto result = m_commandList->Close();//E_FAIL
	if (FAILED(result))
	{
		return false;
	}
...
ID3D12GraphicsCommandList* D3DClass::GetCommandList()
{
