...
ID3D12RootSignature* LightShaderClass::GetSignature()
{
	return m_prtSignature;
}

ID3D12PipelineState* LightShaderClass::GetPSO()
{
...
bool LightShaderClass::InitializeShader(ID3D12Device* device, HWND hwnd, const WCHAR* filename)
{
	...
	result = device->CreateDescriptorHeap(&scvHeapDesc, IID_PPV_ARGS(&m_scvHeap));
	if (FAILED(result))
	{
		return false;
	}
	...
	device->CreateShaderResourceView(m_comTexture, &srvDesc, m_scvHeap->GetCPUDescriptorHandleForHeapStart());//We don't need an offset for the first view on the heap

	// Setup the description of the dynamic matrix constant buffer that is in the vertex shader.
	constantBufferDesc.BufferLocation = m_matrixBuffer->GetGPUVirtualAddress();
	constantBufferDesc.SizeInBytes = (sizeof(MatrixBufferType) / 256 + 1) * 256;//Must be rounded up to 256 or the device will be removed!
	...
	handle.ptr = m_scvHeap->GetCPUDescriptorHandleForHeapStart().ptr + offset;
	...
	rtSignatureDesc.NumParameters = 4;
